...
def visualize_graph(graph, filename: str = "instruction_graph") -> None:
    try:
        ...
        action_color = '#ABEBC6'
        object_color = '#F9E79F'
        subject_color = '#F9E79F'
        initial_color = '#F5B7B1'
        final_color = '#D5D8DC'
        ...
        for state_id, state in graph.states.items():
            ...
            G.add_node(f"state_{state_id}", label=label, type='state', color=color)
        
        for action_id, action in graph.actions.items():
            G.add_node(f"action_{action_id}", label=f"{action.name[:30]}", type='action', color=action_color)
        ...
        for obj_name, obj in graph.objects.items():
            if obj.obj_type == "subject":
                ...
                label = f"{obj_name}\n(объект)"
                color = object_color
            G.add_node(f"obj_{obj_name}", label=label, type=node_type, color=color)
        ...
        for action_id, action in graph.actions.items():
            for obj_name in action.required_objects:
                if obj_name in subjects:
                    G.add_edge(f"obj_{obj_name}", f"action_{action_id}", edge_type='subject', style='dashed')
                else:
                    G.add_edge(f"action_{action_id}", f"obj_{obj_name}", edge_type='object', style='dashed')
        ...
        for node_type, color in [('state', state_color), ('action', action_color), ('object', object_color), ('subject', subject_color)]:
            nodes = [n for n, d in G.nodes(data=True) if d.get('type') == node_type]
            if nodes:
                nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=color, node_size=2800,
                                       edgecolors='black', linewidths=1.5, alpha=0.9, ax=ax)
        initial_nodes = [f"state_{sid}" for sid, s in graph.states.items() if s.state_type.value == "initial"]
        final_nodes = [f"state_{sid}" for sid, s in graph.states.items() if s.state_type.value == "final"]
        if initial_nodes:
            nx.draw_networkx_nodes(G, pos, nodelist=initial_nodes, node_color=initial_color, node_size=2800,
                                   edgecolors='black', linewidths=2, alpha=0.9, ax=ax)
        if final_nodes:
            nx.draw_networkx_nodes(G, pos, nodelist=final_nodes, node_color=final_color, node_size=2800,
                                   edgecolors='black', linewidths=2, alpha=0.9, ax=ax)
        
        def draw_arrow_with_offset(ax, start, end, color='gray', linewidth=1.5, linestyle='-', alpha=0.7, offset=0.2):
            x1, y1 = pos[start]
            x2, y2 = pos[end]
            dx = x2 - x1
            dy = y2 - y1
        ...
        for u, v, d in G.edges(data=True):
            ...
            if edge_type == 'transition':
                draw_arrow_with_offset(ax, u, v, color='gray', linewidth=1.5, linestyle='-', offset=0.2)
            elif edge_type == 'subject':
                draw_arrow_with_offset(ax, u, v, color='blue', linewidth=1.5, linestyle='--', offset=0.2)
            elif edge_type == 'object':
                draw_arrow_with_offset(ax, u, v, color='green', linewidth=1.5, linestyle='--', offset=0.2)
        
        labels = nx.get_node_attributes(G, 'label')
        for node, (x, y) in pos.items():
            label = labels.get(node, '')
            ax.text(x, y, label, fontsize=8, fontweight='bold', ha='center', va='center', zorder=3,
        ...
        legend_elements = [
            ...
            Patch(facecolor=object_color, alpha=0.8, edgecolor='black', label='Объекты (направление: действие → объект)'),
            Patch(facecolor=subject_color, alpha=0.8, edgecolor='black', label='Субъекты (направление: субъект → действие)'),
            ...
            Line2D([0], [0], color='gray', linewidth=1.5, marker='>', markersize=10, label='Переход (сплошная стрелка)'),
            Line2D([0], [0], color='blue', linewidth=1.5, linestyle='--', marker='>', markersize=10, label='Субъект → Действие (синий пунктир)'),
            Line2D([0], [0], color='green', linewidth=1.5, linestyle='--', marker='>', markersize=10, label='Действие → Объект (зеленый пунктир)')
        ]
        ax.legend(handles=legend_elements, loc='upper left', fontsize=9, framealpha=0.9, bbox_to_anchor=(1.02, 0.5))
        ax.set_title(f"Граф инструкций: {graph.name}", fontsize=14, fontweight='bold')
        ax.axis('off')
        ax.set_aspect('equal')
